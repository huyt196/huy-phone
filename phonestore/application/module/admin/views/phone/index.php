<?php 
	...
	$lblCategory	= Helper::cmsLinkSort('Category-Phone', 'category_phone_id', $columnPost, $orderPost);
	...
	// Pagination
	$paginationHTML		= $this->pagination->showPagination(URL::createLink('admin', 'phone', 'index'));
				...
				<?php 
							if(!empty($this->Items)){
								...
								foreach($this->Items as $key => $value){
									...
									$name		= $value['name'];
									$picture 	= Helper::createImage('phone', '', $value['picture'], array('width' => 60, 'height' => 90));
									$price		= $value['price'];
									$saleoff	= $value['sale_off'];
									$categoryName	= $value['category_name'];
									$row		= ($i % 2 == 0) ? 'row0' : 'row1';
									$status		= Helper::cmsStatus($value['status'], URL::createLink('admin', 'phone', 'ajaxStatus', array('id' => $id, 'status' => $value['status'])), $id);
									$special	= Helper::cmsSpecial($value['special'], URL::createLink('admin', 'phone', 'ajaxSpecial', array('id' => $id, 'special' => $value['special'])), $id);
									$ordering	= '<input type="text" name="order['.$id.']" size="5" value="'.$value['ordering'].'" class="text-area-order">';
									$created	= Helper::formatDate('d-m-Y', $value['created']);
									$created_by	= $value['created_by'];
									$modified	= Helper::formatDate('d-m-Y', $value['modified']);
									$modified_by= $value['modified_by'];
									$linkEdit	= URL::createLink('admin', 'phone', 'form', array('id' => $id));
