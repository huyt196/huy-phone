<?php 
	...
	$lblName 		= Helper::cmsLinkSort('FullName', 'name', $columnPost, $orderPost);
	$lblWork 		= Helper::cmsLinkSort('Job', 'work', $columnPost, $orderPost);
	$lblAvatar		= Helper::cmsLinkSort('Avatar', 'avatar', $columnPost, $orderPost);
	$lblContent		= Helper::cmsLinkSort('Content', 'content', $columnPost, $orderPost);
	$lblStar 		= Helper::cmsLinkSort('Star', 'star', $columnPost, $orderPost);
	...
	// Pagination
	$paginationHTML		= $this->pagination->showPagination(URL::createLink('admin', 'experience', 'index'));
                    ...
                    <table class="adminlist" id="modules-mgr">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th width="10%"><?php echo $lblAvatar;?></th>
                                <th width="10%" class="title"><?php echo $lblName;?></th>
                                <th width="15%"><?php echo $lblWork;?></th>
                                <th width="30%"><?php echo $lblContent;?></th>
                                <th width="10%"><?php echo $lblStar;?></th>
				...
				<?php 
							if(!empty($this->Items)){
								...
								foreach($this->Items as $key => $value){
									...
									$name		= $value['name'];
									$avatar 	= Helper::createImage('experience', '', $value['avatar'], array('width' => 60, 'height' => 90));
									$work		= $value['work'];
									$content	= $value['content'];
                                    $star	= $value['star'];
									$row		= ($i % 2 == 0) ? 'row0' : 'row1';
									$status		= Helper::cmsStatus($value['status'], URL::createLink('admin', 'experience', 'ajaxStatus', array('id' => $id, 'status' => $value['status'])), $id);
									$ordering	= '<input type="text" name="order['.$id.']" size="5" value="'.$value['ordering'].'" class="text-area-order">';
									$created	= Helper::formatDate('d-m-Y', $value['created']);
									$created_by	= $value['created_by'];
									$modified	= Helper::formatDate('d-m-Y', $value['modified']);
									$modified_by= $value['modified_by'];
									$linkEdit	= URL::createLink('admin', 'experience', 'form', array('id' => $id));
