...
class BlogModel extends Model{
	...
	public function listItem($arrParam, $option = null){
		$query[]	= "SELECT `b`.`id`,  `b`.`name`, `b`.`picture`, `b`.`status`, `b`.`ordering`, `b`.`created`, `b`.`created_by`, `b`.`modified`, `b`.`modified_by`";
	...
	public function changeStatus($arrParam, $option = null){
		...
		if($option['task'] == 'change-ajax-special'){
			...
			$result = array(
					'id'		=> $id,
					'special'	=> $special,
					'link'		=> URL::createLink('admin', 'blog', 'ajaxSpecial', array('id' => $id, 'special' => $special))
			);
			return $result;
		}
	...
	public function saveItem($arrParam, $option = null){
		...
		if($option['task'] == 'add'){
			$arrParam['form']['picture']	= $uploadObj->uploadFile($arrParam['form']['picture'], 'blog', 98, 150);
