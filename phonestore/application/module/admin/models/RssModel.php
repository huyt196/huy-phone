...
class RssModel extends Model{
	...
	private $_columns = array('id', 'link',  'status', 'ordering');
	private $_userInfo;
	...
	public function changeStatus($arrParam, $option = null){
		if($option['task'] == 'change-ajax-status'){
			...
			$result = array(
								'id'		=> $id, 
								'status'	=> $status, 
								'link'		=> URL::createLink('admin', 'rss', 'ajaxStatus', array('id' => $id, 'status' => $status))
						); 
			return $result;
		}
		...
		if($option['task'] == 'change-ajax-group-acp'){
			...
			$result = array(
					'id'		=> $id,
					'group_acp'	=> $group_acp,
					'link'		=> URL::createLink('admin', 'rss', 'ajaxACP', array('id' => $id, 'group_acp' => $group_acp))
			);
			return $result;
		}
