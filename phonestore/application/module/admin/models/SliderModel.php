...
class SliderModel extends Model{
	...
	private $_columns = array('id', 'name', 'description', 'picture','created', 'created_by', 'modified', 'modified_by', 'status', 'ordering', 'category_phone_id');
	private $_userInfo;
	...
	public function itemInSelectbox($arrParam, $option = null){
		if($option == null){
			$query 	= "SELECT `id`, `name` FROM `" . TBL_CATEGORYPHONE . "`";
	...
	public function listItem($arrParam, $option = null){
		$query[]	= "SELECT `b`.`id`, `b`.`name`, `b`.`picture`, `b`.`status`, `b`.`ordering`, `b`.`created`, `b`.`created_by`, `b`.`modified`, `b`.`modified_by`, `c`.`name` AS `category_name`";
		$query[]	= "FROM `$this->table` AS `b` LEFT JOIN `". TBL_CATEGORYPHONE . "` AS `c` ON `b`.`category_phone_id` = `c`.`id`";
		$query[]	= "WHERE `b`.`id` > 0";
	...
	public function changeStatus($arrParam, $option = null){
		if($option['task'] == 'change-ajax-status'){
			...
			$result = array(
								'id'		=> $id, 
								'status'	=> $status, 
								'link'		=> URL::createLink('admin', 'slider', 'ajaxStatus', array('id' => $id, 'status' => $status))
						); 
			return $result;
		}
		...
		if($option['task'] == 'change-ajax-special'){
			...
			$result = array(
					'id'		=> $id,
					'special'	=> $special,
					'link'		=> URL::createLink('admin', 'slider', 'ajaxSpecial', array('id' => $id, 'special' => $special))
			);
			return $result;
		}
		...
		if($option == null){
			if(!empty($arrParam['cid'])){
				...
				foreach ($arrImage as $value){
					...
					$uploadObj->removeFile('slider', '98x150-' . $value);
				}
	...
	public function infoItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `description`, `picture`,`name`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
	...
	public function saveItem($arrParam, $option = null){
		...
		if($option['task'] == 'add'){
			$arrParam['form']['picture']	= $uploadObj->uploadFile($arrParam['form']['picture'], 'slider');
			$arrParam['form']['created']	= date('Y-m-d', time());
			$arrParam['form']['created_by']	= $this->_userInfo['username'];


			$arrParam['form']['description']    = mysqli_real_escape_string($this->getConnect(),$arrParam['form']['description']);
		...
		if($option['task'] == 'edit'){
			if($arrParam['form']['picture']['name']==null){
				...
				$uploadObj->removeFile('slider', $arrParam['form']['picture_hidden']);
				$uploadObj->removeFile('slider', '98x150-' .  $arrParam['form']['picture_hidden']);
