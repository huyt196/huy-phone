...
class PhoneModel extends Model{
	...
	public function changeStatus($arrParam, $option = null){
		if($option['task'] == 'change-ajax-status'){
			...
			$result = array(
								'id'		=> $id, 
								'status'	=> $status, 
								'link'		=> URL::createLink('admin', 'phone', 'ajaxStatus', array('id' => $id, 'status' => $status))
						); 
			return $result;
		}
		...
		if($option['task'] == 'change-ajax-special'){
			...
			$result = array(
					'id'		=> $id,
					'special'	=> $special,
					'link'		=> URL::createLink('admin', 'phone', 'ajaxSpecial', array('id' => $id, 'special' => $special))
			);
			return $result;
		}
		...
		if($option == null){
			if(!empty($arrParam['cid'])){
				...
				foreach ($arrImage as $value){
					$uploadObj->removeFile('phone', $value);
					$uploadObj->removeFile('phone', '98x150-' . $value);
				}
	...
	public function infoItem($arrParam, $option = null){
		if($option == null){
			$query[]	= "SELECT `id`, `description`, `picture`,`name`, `price`, `special`,`sale_off`,`category_phone_id`, `status`, `ordering`";
			$query[]	= "FROM `$this->table`";
	...
	public function saveItem($arrParam, $option = null){
		...
		if($option['task'] == 'add'){
			$arrParam['form']['picture']	= $uploadObj->uploadFile($arrParam['form']['picture'], 'phone');
		...
		if($option['task'] == 'edit'){
			if($arrParam['form']['picture']['name']==null){
				...
				$uploadObj->removeFile('phone', $arrParam['form']['picture_hidden']);
				$uploadObj->removeFile('phone', '98x150-' .  $arrParam['form']['picture_hidden']);
