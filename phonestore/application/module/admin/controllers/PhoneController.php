...
class PhoneController extends Controller{
	...
	public function indexAction(){
		$this->_view->_title 		= 'Phone Manager :: List';
		...
		$this->_view->render('phone/index');
	}
	
	// ACTION: ADD & EDIT BOOK
	public function formAction(){
		$this->_view->_title = 'Phone : Add';
		$this->_view->slbCategory		= $this->_model->itemInSelectbox($this->_arrParam, null);
		if(!empty($_FILES)) $this->_arrParam['form']['picture']  = $_FILES['picture'];
		
		if(isset($this->_arrParam['id'])){
			$this->_view->_title = 'Phone : Edit';
			$this->_arrParam['form'] = $this->_model->infoItem($this->_arrParam);
			if(empty($this->_arrParam['form'])) URL::redirect('admin', 'phone', 'index');
		}
		...
		if($this->_arrParam['form']['token'] > 0){
			...
			$validate->addRule('name', 'string', array('min' => 1, 'max' => 255))
					 ...
					 ->addRule('category_phone_id', 'status', array('deny' => array('default')))
			...
			if($validate->isValid() == false){
				...
				if($this->_arrParam['type'] == 'save-close') 	URL::redirect('admin', 'phone', 'index');
				if($this->_arrParam['type'] == 'save-new') 		URL::redirect('admin', 'phone', 'form');
				if($this->_arrParam['type'] == 'save') 			URL::redirect('admin', 'phone', 'form', array('id' => $id));
			}
		}
		
		$this->_view->arrParam = $this->_arrParam;
		$this->_view->render('phone/form');
	}
	...
	public function statusAction(){
		...
		URL::redirect('admin', 'phone', 'index');
	}
