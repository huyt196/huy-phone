...
class Bootstrap{
	...
	public function init(){
		...
		if(file_exists($filePath)){
			...
			URL::redirect('phone', 'index', 'notice', array('type' => 'not-url'));
		}
	}
			...
			if($module == 'admin'){
				if($logged == true){
					if($userInfo['group_acp'] == 1){
						...
						//}else{
						//	URL::redirect('default2', 'index', 'notice', array('type' => 'not-permission'));
						//}
					}else{
						URL::redirect('phone', 'index', 'notice', array('type' => 'not-permission'));
					}
				}else{
	...
	// CALL ACTION LOGIN
	private function callLoginAction($module = 'phone'){
		Session::delete('user');
	...
	public function _error(){
		require_once MODULE_PATH . 'phone' . DS . 'controllers' . DS . 'ErrorController.php';
		$this->_controllerObject = new ErrorController();
		$this->_controllerObject->setView('phone');
