...
class Pagination
{
	...
	public function showPagination($link, $option = null)
	{
		if ($option == null) {
			// Pagination
			$paginationHTML = '';
		...
		if ($option['phone'] == true) {
			// Pagination
			$paginationHTML = '';
			if ($this->totalPage > 1) {
				$prev 	= ' <li class="page-item disabled"><a class="page-link" href="#" tabindex="-1"><i class="linearicons-arrow-left"></i></a></li>';
				if ($this->currentPage > 1) {
					$prev 	= '<li class="page-item"><a class="page-link" onclick="javascript:changePage(' . ($this->currentPage - 1) . ')" href="#"><i class="linearicons-arrow-left"></i></a></li>';
				}

				$next 	= '<li class="page-item disabled"><a class="page-link" href="#"><i class="linearicons-arrow-right"></i></a></li>';
				
				if ($this->currentPage < $this->totalPage) {
					$next 	= '<li class="page-item"><a class="page-link" onclick="javascript:changePage(' . ($this->currentPage + 1) . ')" href="#"><i class="linearicons-arrow-right"></i></a></li>';
				}
				...
				for ($i = $startPage; $i <= $endPage; $i++) {
					if ($i == $this->currentPage) {
						...
						$listPages .= '<li class="page-item"><a class="page-link" href="#" onclick="javascript:changePage(' . $i . ')">' . $i . '</a></li>';
					}
				}
				$listPages .= '';
				$endPagination	= '<div class="limit">Page ' . $this->currentPage . ' of ' . $this->totalPage . '</div>';
